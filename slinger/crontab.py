...
ENTRIES = len(_ranges)
SECOND_OFFSET, MINUTE_OFFSET, HOUR_OFFSET, DAY_OFFSET, MONTH_OFFSET, WEEK_OFFSET, YEAR_OFFSET = list(range(ENTRIES))
...
class _Matcher(object):
    ...
    def __call__(self, v, dt):
        for i, x in enumerate(self.split):
            if x == 'l':
                ...
                start, end = list(map(int, x.partition('-')[::2]))
    ...
    def _parse_crontab(self, which, entry):
        ...
        def _parse_piece(it):
            if '-' in it:
                start, end = list(map(_fix, it.split('-')))
                # Allow "sat-sun"
...
class CronTab(object):
    ...
    def next(self, now=None, increments=_increments, delta=True, default_utc=WARN_CHANGE):
        ...
        # verify the match
        match = [self._test_match(i, future) for i in range(ENTRIES)]
        _assert(all(match),
