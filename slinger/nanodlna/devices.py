...
UPNP_RENDER_CONTROL_TYPE = "urn:schemas-upnp-org:service:RenderingControl:1"
...
def register_device(location_url):
    ...
    render_control_url_path = get_xml_field_text(
        device_root,
        "./serviceList/service/"
        "[serviceType='{0}']/controlURL".format(
            UPNP_RENDER_CONTROL_TYPE
        )
    )

    if render_control_url_path is not None:
        render_control_url_path = urllibparse.urljoin(location_url, render_control_url_path)
    else:
        render_control_url_path = None
    ...
    device = {
        ...
        "render_control_url" : render_control_url_path,
        "render_control_st": UPNP_RENDER_CONTROL_TYPE
    }
